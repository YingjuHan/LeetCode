...
public:
    ...
    vector<vector<int>> highestRankedKItems(vector<vector<int>>& grid, vector<int>& pricing, vector<int>& start, int k) {
        ...
        int m = grid.size(), n = grid[0].size(), low = pricing[0], high = pricing[1], sx = start[0], sy = start[1];
        ...
        while (!q.empty()) {
            ...
            vector<pair<int, int>> temp_q;
            for (auto& [x, y] : q) {
                for (auto& [dx, dy] : DIRS) {
                    int nx = x + dx, ny = y + dy;
                    if (nx >= 0 && nx < m && ny >= 0 && ny < n && vis[nx][ny] == 0 && grid[nx][ny] != 0) {
                        vis[nx][ny] = 1;
                        temp_q.push_back({nx, ny});
                    }
                }
            }
            q = move(temp_q);
