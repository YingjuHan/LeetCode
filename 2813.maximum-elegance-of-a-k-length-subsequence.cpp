    ...
    long long findMaximumElegance(vector<vector<int>>& items, int k) {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (i < k) {
                ...
                if (!vis.insert(category).second) { // 插入不成功即为重复类别
                    duplocale.push(profit);
                }
            } else if (!duplocale.empty() && vis.insert(category).second) {
                total_profit += profit - duplocale.top(); // 使用当前利润替代之前的一个重复类别中的最小利润
                duplocale.pop();
            }
            res = max(res, total_profit + (long long)vis.size() * (long long)vis.size());
        }
        return res;
    }
};
