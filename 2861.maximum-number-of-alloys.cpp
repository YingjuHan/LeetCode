 ...
 * 
 * 对于第 i 台机器而言，创建合金需要 composition[i][j] 份 j 类型金属。最初，你拥有 stock[x] 份 x
 * 类型金属，而每购入一份 x 类型金属需要花费 cost[x] 的金钱。
 * 
...
public:
    int maxNumberOfAlloys(int n, int k, int budget, vector<vector<int>>& composition, vector<int>& stock, vector<int>& cost) {
        ...
        for (auto& comp : composition) {
            ...
            // 假设 composition[i][j] 和 cost[j] 都是 1，且只有一种金属需要购买，我们可以制造的合金数最多为 min(stock)+budget
            int left = res, right = *min_element(stock.begin(), stock.end()) + budget + 1;
            while (left + 1 < right) {
