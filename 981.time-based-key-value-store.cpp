...
public:
    ...
    void set(string key, string value, int timestamp) {
        ump[key].emplace_back(timestamp, value);
    }
    
    string get(string key, int timestamp) {
        auto& umpVal = ump[key];
        int left = lower_bound(umpVal, timestamp + 1);
        if (left == 0) {
            return "";
        }
        return umpVal[left - 1].second;
    }

    int lower_bound(vector<pair<int, string>>& valVec, int target) {
        int left = 0, right = valVec.size() - 1;
        while (left <= right) {
            int mid = left + (right - left) / 2;
            if (valVec[mid].first < target) {
                left = mid + 1;
            } else {
                right = mid - 1;
            }
