/*
 * @lc app=leetcode.cn id=LCP 68 lang=cpp
 * @lcpr version=30204
 *
 * [LCP 68] 美观的花束
 *
 ...
 * > - 例如：`[5,5,5,6,6]` 中品种为 `5` 的花有 `3` 朵， 品种为 `6` 的花有 `2` 朵，**每一品种** 的数量均不超过
 * `3`
 * 
...
public:
    int beautifulBouquet(vector<int>& flowers, int cnt) {
        ...
        while (right < flowers.size()) {
            int in_win = flowers[right];
            cntMp[in_win]++;
            right++;
            while (cntMp[in_win] > cnt) {
                int out_win = flowers[left];
                cntMp[out_win]--;
                left++;
            }
            res += right - left;
